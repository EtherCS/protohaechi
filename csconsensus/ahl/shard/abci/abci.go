...
import (
	...
	ahlNode "github.com/tendermint/tendermint/csconsensus/ahl/shard/validator"
)
	...
	if tx_json.Tx_type == ahlNode.IntraShard_TX {
		...
		if app.Node.Byzantine == 1 && string(new_tx.From) == "VCTM" { // construct front-running tx: new_tx1
			if string(new_tx.To) == "INTR" { // front-running attack with intra-shard txs
				new_tx1.From_shard = new_tx.To_shard
				new_tx1.Tx_type = ahlNode.IntraShard_TX

			} else if string(new_tx.To) == "CROS" { // front-running attack with cross-shard txs
				new_tx1.From_shard = app.Node.Attack_shard
				new_tx1.Tx_type = ahlNode.InterShard_TX_Verify
			}
			new_tx1.Nonce = uint32(app.Node.Node_id)
			new_tx1.From = []byte("ATTA")
			_, attack_tx := ahlNode.Deserilization(new_tx1)
