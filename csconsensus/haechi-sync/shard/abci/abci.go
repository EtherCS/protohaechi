...
/*
Implementation of Haechi with synchronization mechanism
Basic idea: make block invalid and not send msgs to the beacon chain if didn't synchronize (i.e., receive msg from the beacon chain)
valiable 'synchronize' is the key of implementation
*/
import (
	"encoding/binary"
	"fmt"
	...
	haechiSyncNode "github.com/tendermint/tendermint/csconsensus/haechi-sync/shard/validator"
...
type HaechiShardApplication struct {
	...
	consensusGuard sync.Mutex
	synchronize    bool
	Node           *haechiSyncNode.ValidatorInterface
}

func NewHaechiShardApplication(node *haechiSyncNode.ValidatorInterface) *HaechiShardApplication {
...
func (app *HaechiShardApplication) CheckTx(req abcitypes.RequestCheckTx) abcitypes.ResponseCheckTx {
	_, tx_json := haechiSyncNode.ResolveTx(req.Tx)
	if tx_json.Tx_type == haechiSyncNode.CrossShard_Call_List {
		fmt.Println(fmt.Sprintf("debug unlock at %v", time.Now()))
...
func (app *HaechiShardApplication) DeliverTx(req abcitypes.RequestDeliverTx) abcitypes.ResponseDeliverTx {
	...
	if !app.synchronize { // skip transaction processing
		goto INVALID
	}
	// fmt.Println("shard receives tx type " + strconv.Itoa(int(tx_json.Tx_type)))

	if tx_json.Tx_type == haechiSyncNode.IntraShard_TX {
		// Attacking test
	...
	} else if tx_json.Tx_type == haechiSyncNode.InterShard_TX_Verify {
		...
		if app.Node.Byzantine == 1 && string(temp_tx.From) == "VCTM" { // construct front-running tx: new_tx1
			if string(temp_tx.To) == "INTR" { // front-running attack with intra-shard txs
				...
				temp_tx.Tx_type = haechiSyncNode.InterShard_TX_Verify
			...
			_, attack_tx := haechiSyncNode.Deserilization(temp_tx)
			go app.Node.DeliverAttackTx(attack_tx, temp_tx.To_shard) // sending front-running tx
		}
	} else if tx_json.Tx_type == haechiSyncNode.CrossShard_Call_List {
		// fmt.Println("shard receives call list")
		go app.Node.HandleCallList(req.Tx)
	} else if tx_json.Tx_type == haechiSyncNode.InterShard_TX_Commit {
		...
		tx_json.Tx_type = haechiSyncNode.InterShard_TX_Update
		_, new_tx := haechiSyncNode.Deserilization(tx_json)
		if app.Node.Leader {
			go app.Node.DeliverUpdateTx(new_tx, tx_json.To_shard)
			// app.Node.DeliverUpdateTx(new_tx, tx_json.To_shard)
		}
	} else if tx_json.Tx_type == haechiSyncNode.InterShard_TX_Update {
...
func (app *HaechiShardApplication) Commit() abcitypes.ResponseCommit {
	...
	if app.Node.Leader && app.Node.Current_cl != "" {
		if app.synchronize { // valid to send ccl
