...
type CrossLinkTransaction struct {
...
func RequestToCrossLinkTxs(ctx []byte) (uint32, []CrossLinkTransaction) { // transfer a crosslink to a list of crosslink transaction
	var cls []CrossLinkTransaction
	var blockts int64
	reqs := bytes.Split(ctx, []byte(">"))
	cls = make([]CrossLinkTransaction, len(reqs)-1)
	for i := uint(0); i < uint(len(reqs)-1); i++ {
		req := reqs[i]
		txElements := bytes.Split(req, []byte(","))
		var cl CrossLinkTransaction
...
func CrossLinkToTx(cl CrossLinkTransaction) TransactionType {
