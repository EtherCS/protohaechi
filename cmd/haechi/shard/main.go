...
import (
	...
	hshardapp "github.com/tendermint/tendermint/csconsensus/haechi/shard/abci"
	hshardnode "github.com/tendermint/tendermint/csconsensus/haechi/shard/validator"
...
func main() {
	...
	if isLeader == "true" {
		validatorInterface = hshardnode.NewValidatorInterface(db, uint8(shardNum), uint8(shardid), true, in_ip_temp, out_ips_temps, uint8(isByzantine), uint8(attackShard), uint8(nodeID))
	} else if isLeader == "false" {
		validatorInterface = hshardnode.NewValidatorInterface(db, uint8(shardNum), uint8(shardid), false, in_ip_temp, out_ips_temps, uint8(isByzantine), uint8(attackShard), uint8(nodeID))
	}
