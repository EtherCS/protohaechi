...
func main() {
	...
	go http.Get(fmt.Sprintf("http://%v:%v/broadcast_tx_commit?tx=\"fromid=%v,toid=%v,type=%v,from=CROS,to=WXYZ,value=10,data=NONE,nonce=%v\"", shardIp, shardPort, 1, 0, 1, nonce))

	// intra-shard latency
	intra_start := time.Now()
	// fmt.Println("intra-shard start in:", intra_start)
	intra_req := fmt.Sprintf("http://%v:%v/broadcast_tx_commit?tx=\"fromid=%v,toid=%v,type=%v,from=INTR,to=EFGH,value=10,data=NONE,nonce=%v\"", shardIp, shardPort, 0, 0, 0, get_rand(math.MaxInt32))
	http.Get(intra_req)
