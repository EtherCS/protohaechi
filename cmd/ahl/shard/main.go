...
import (
	...
	ahlshardapp "github.com/tendermint/tendermint/csconsensus/ahl/shard/abci"
	ahlshardnode "github.com/tendermint/tendermint/csconsensus/ahl/shard/validator"
...
func main() {
	...
	if isLeader == "true" {
		validatorInterface = ahlshardnode.NewValidatorInterface(db, uint8(shardNum), uint8(shardid), true, in_ip_temp, out_ips_temps, uint8(isByzantine), uint8(attackShard), uint8(nodeID))
	} else if isLeader == "false" {
		validatorInterface = ahlshardnode.NewValidatorInterface(db, uint8(shardNum), uint8(shardid), false, in_ip_temp, out_ips_temps, uint8(isByzantine), uint8(attackShard), uint8(nodeID))
	}
