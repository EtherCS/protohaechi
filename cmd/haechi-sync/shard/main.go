...
import (
	...
	hshardsyncapp "github.com/tendermint/tendermint/csconsensus/haechi-sync/shard/abci"
	hshardsyncnode "github.com/tendermint/tendermint/csconsensus/haechi-sync/shard/validator"
...
func main() {
	...
	db := hshardsyncnode.NewBlockchainState("leveldb", dbPath)
	defer func() {
		if err := db.Database.Close(); err != nil {
			log.Fatalf("Closing database: %v", err)
		}
	}()
	var validatorInterface *hshardsyncnode.ValidatorInterface
	...
	if isLeader == "true" {
		validatorInterface = hshardsyncnode.NewValidatorInterface(db, uint8(shardNum), uint8(shardid), true, in_ip_temp, out_ips_temps, uint8(isByzantine), uint8(attackShard), uint8(nodeID))
	} else if isLeader == "false" {
		validatorInterface = hshardsyncnode.NewValidatorInterface(db, uint8(shardNum), uint8(shardid), false, in_ip_temp, out_ips_temps, uint8(isByzantine), uint8(attackShard), uint8(nodeID))
	}

	app := hshardsyncapp.NewHaechiShardApplication(validatorInterface)
