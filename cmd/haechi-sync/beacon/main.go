...
import (
	...
	hbcchainsyncapp "github.com/tendermint/tendermint/csconsensus/haechi-sync/coordinator/abci"
	hbcchainsyncnode "github.com/tendermint/tendermint/csconsensus/haechi-sync/coordinator/validator"
...
func main() {
	...
	db := hbcchainsyncnode.NewBlockchainState("leveldb", dbPath)
	defer func() {
		if err := db.Database.Close(); err != nil {
			log.Fatalf("Closing database: %v", err)
		}
	}()
	var validatorInterface *hbcchainsyncnode.ValidatorInterface
	...
	if isLeader == "true" {
		validatorInterface = hbcchainsyncnode.NewValidatorInterface(db, uint8(shardNum), true, in_ip_temp, out_ips_temps)
	} else if isLeader == "false" {
		validatorInterface = hbcchainsyncnode.NewValidatorInterface(db, uint8(shardNum), false, in_ip_temp, out_ips_temps)
	}

	app := hbcchainsyncapp.NewHaechiBeaconApplication(validatorInterface)
