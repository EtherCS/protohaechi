...
WORKSPACE="$GOSRC/github.com/EtherCS/haechi"
...
BEACON_PORT=10057
BEACON_IP="127.0.0.1"
SHARD_PORTS="20057,21057,22057,23057,24057,25057,26057,27057"
SHARD_IPS="127.0.0.1,127.0.0.1,127.0.0.1,127.0.0.1,127.0.0.1,127.0.0.1,127.0.0.1,127.0.0.1"
...
cp -r configs/single-machine/haechi-sync/* $TM_HOME
...
do
    ...
    do
        if [ $k -eq 0 ]; then
            ...
            ./build/haechisyncshard -home $TM_HOME/shard$j/node$k -leader "false" -shards $SHARD_NUM -shardid $j -beaconport $BEACON_PORT -shardports $SHARD_PORTS -beaconip $BEACON_IP -shardips $SHARD_IPS &> $LOG_DIR/shard$j-node$k.log &
        fi
    sleep 1
    done
done
