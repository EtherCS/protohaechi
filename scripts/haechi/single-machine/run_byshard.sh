...
do
    ...
    do
        if [ $k -eq 0 ]; then
            ...
            ./build/byshard -home $TM_HOME/shard$j/node$k -leader "false" -shards $SHARD_NUM -shardid $j -beaconport $BEACON_PORT -shardports $SHARD_PORTS -beaconip $BEACON_IP -shardips $SHARD_IPS -byzantine 1 -attackid 1 -nodeid $j &> $LOG_DIR/shard$j-node$k.log &
        fi
    sleep 1
    done
done
